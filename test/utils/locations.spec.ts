import { distanceInKmBetweenEarthCoordinates, getClosestCounty } from "../../src/utils/location"
...
describe("distanceInKmBetweenEarthCoordinates", () => {
  it("returns zero for equal coords", () => {
    ...
    expect(distanceInKmBetweenEarthCoordinates(p1, p2)).toEqual(0)
  })

  it("calcs distance between two coords", () => {
    const p1 = { latitude: 1.2345, longitude: 2.3456 }
    const p2 = { latitude: 3.4567, longitude: 4.5676 }
    expect(distanceInKmBetweenEarthCoordinates(p1, p2)).toBeGreaterThan(349.27)
    expect(distanceInKmBetweenEarthCoordinates(p1, p2)).toBeLessThanOrEqual(349.29)
  })
})
...
describe("getClosestCounty", () => {
  it("returns the closest County to a location", () => {
    ...
    const expectedResult = {
      county: county2,
      distance: distanceInKmBetweenEarthCoordinates(county2.gpsLocation, location),
    }
